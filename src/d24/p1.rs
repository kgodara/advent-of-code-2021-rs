pub fn exec(src: String) {
    ...
    for region_pair in store_extract_pairs.iter() {
        // find max pair
        let mut max_lower_bound = 0;
        // find highest lower-bound for range of two single-digit values separated by (region_pair.1)
        for lower_bound in 1..10 {
            // NOTE: not >= 0, since 0 digits not allowed
            if (lower_bound + region_pair.1) > 0 && (lower_bound + region_pair.1) < 10 { max_lower_bound = lower_bound; }
        }
        result[region_pair.0.1 as usize] = (max_lower_bound + region_pair.1) as u8;
        result[region_pair.0.0 as usize] = max_lower_bound as u8;
    }
