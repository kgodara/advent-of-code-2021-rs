...
fn attempt_direct_move(hallway: &mut Vec<u8>, rooms: &mut Vec<Vec<u8>>, cost: &mut u64) -> bool {
    ...
    let candidate_end_rooms: Vec<(usize, usize)> = rooms.iter()
        ...
        .filter_map(|(room_idx, room)| {
            // bottom spot empty (assumption: top spot never occupied if bottom empty)
            if room[1] == 0 {
                Some((room_idx, 1))
            }
            // bottom spot has correct member & top spot is empty
            else if room[1] == ((room_idx+1) as u8) && room[0] == 0 {
                Some((room_idx, 0))
            } else {
                None
            }
    ...
    while !state_stack.is_empty() {
        ...
        let can_move_to_hall_list: Vec<(usize, usize)> = cur_rooms.iter()
            ...
            .filter_map(|(room_idx, room)| {
                ...
                let room_cmp_idx = room_idx + 1;
                // if top-occupant is invalid (top needs to move)
                // OR if bottom-occupant is invalid (top needs to move)
                if room[0] != 0 && (room[0] != (room_cmp_idx as u8) || room[1] != (room_cmp_idx as u8)) {
                    Some((room_idx, 0))
                }

                // if top-occupant is empty and bottom-occupant is invalid (bottom needs to move)
                else if room[0] == 0 && room[1] != (room_cmp_idx as u8) && room[1] != 0 {
                    Some((room_idx, 1))
                }
