...
// candidate end-rooms for [hall | room]->room move
...
fn attempt_move_to_hall(cur_state: ([u8; 11], [[u8; 4]; 4], u64),
    valid_spaces: &mut [u8; 7],
    state_stack: &mut Vec<([u8; 11], [[u8; 4]; 4], u64)>) {
...
pub fn exec(src: &str, print: bool) {
    ...
    */
    ...
    for (room_idx, room_occupant) in lines.next().unwrap().trim().replace('#',"").chars().enumerate() {
        rooms[room_idx][3] = val_from_occupant(room_occupant);
    }

    // (hallway, rooms, cost)
    let mut state_stack: Vec<([u8; 11], [[u8; 4]; 4], u64)> = vec![(hallway, rooms, 0)];
