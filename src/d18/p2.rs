...
impl Element {
    fn clone_with_new_alloc(elem: Rc<RefCell<Element>>, new_parent: Option<Rc<RefCell<Element>>>) -> Rc<RefCell<Element>> {
        let new_elem = Rc::new(RefCell::new(Element {
            parent: new_parent,
            value: elem.borrow().value.as_ref().map(|opt| Rc::new(RefCell::new(*opt.borrow())) ),
            children: vec![]
        }));

        if elem.borrow().children.len() == 2 {
            new_elem.borrow_mut().children.push(Element::clone_with_new_alloc(Rc::clone(&elem.borrow().children[0]), Some(Rc::clone(&new_elem))));
            new_elem.borrow_mut().children.push(Element::clone_with_new_alloc(Rc::clone(&elem.borrow().children[1]), Some(Rc::clone(&new_elem))));
        }
...
pub fn exec() {
    ...
    let mut result: Rc<RefCell<Element>>;

    let mut max_mag: u64 = 0;

    for (left_idx, left_num) in input_data.iter().enumerate() {
        for (right_idx, right_num) in input_data.iter().enumerate() {
            ...
            result = Rc::new(RefCell::new(Element {
                parent: None,
                value: None,
                children: vec![ Element::clone_with_new_alloc(Rc::clone(left_num), None), Element::clone_with_new_alloc(Rc::clone(right_num), None) ]
            }));
            ...
            max_mag = std::cmp::max(max_mag, get_magnitude(Rc::clone(&result)))
        }
    }

    println!("result: {}", max_mag);
