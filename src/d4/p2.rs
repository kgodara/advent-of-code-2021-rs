...
fn check_valid_bingo(sol_board: &[Vec<Vec<bool>>],
    ...
    unsolved_boards: &mut HashSet<usize>,
    winner_board_idx: &mut usize) {
    // horizontal|vertical
    ...
    for seq_idx in seqs_to_check.iter() {
        ...
        if !remaining_seq_boards.is_empty() {
            // Note: winner_board_idx only reflects the first determined winner for a move
            *winner_board_idx = remaining_seq_boards.get(0).unwrap().0;

            for board_tup in remaining_seq_boards.iter() {
                unsolved_boards.remove(&board_tup.0);
            }
        }
    }
}
...
pub fn exec() {
    ...
    let mut unsolved_boards: HashSet<usize> = HashSet::new();
    for i in 0..board_num { unsolved_boards.insert(i); }
    ...
    for drawn_num in nums_to_draw.iter() {
        ...
        if let Some(board_hits) = cell_val_lookup.get(drawn_num) {
            // Filter boards_to_mark by unsolved_boards
            boards_to_mark = board_hits.clone()
                .into_iter()
                .filter(|board_cell| unsolved_boards.contains(&board_cell.board_idx))
                .collect();
            ...
            // Search rows/cols
            // Note: also updates unsolved_boards
            check_valid_bingo(&sol_board, true, &rows_to_check, &boards_to_check, &mut unsolved_boards, &mut last_winner_idx);
            if unsolved_boards.is_empty() { break }

            check_valid_bingo(&sol_board, false, &cols_to_check, &boards_to_check, &mut unsolved_boards, &mut last_winner_idx);
            if unsolved_boards.is_empty() { break }
