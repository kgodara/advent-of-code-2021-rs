...
pub fn exec(src: &str, print: bool) {
    ...
    let mut visited_small_cave_stack: Vec<(Rc<RefCell<Cave>>, u16)> = Vec::new();

    let start_cave = cave_lookup.get_mut(&0).unwrap();

    visited_small_cave_stack.push((Rc::clone(start_cave), 0));
    ...
    while !visited_small_cave_stack.is_empty() {
        ...
        for adj_cave in cur_cave.adj_caves.iter() {
            ...
            if (adj_cave_b.n_type == NodeType::SmallCave && !is_visited(&cur_cave_visited, &adj_cave_b.name)) ||
                adj_cave_b.n_type == NodeType::BigCave {

                visited_small_cave_stack.push(( Rc::clone(adj_cave), cur_cave_visited ));
            }
